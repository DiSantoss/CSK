...
italianFood.map(function map(item,index){
    /*dando clone*/
...
/*deveremos cancelar o evento original do elemento, que é a tag a, vamos selecionar e adicionar um evento */
    ...
    /*preenchendo as informações gerais ao clicar  em cada pizza aqui abaixo */
        ...
        c('.pizzaBig img').src = italianFood[key].img


   c('.pizzaInfo h1').innerHTML = italianFood[key].name;


   c('.pizzaInfo--desc').innerHTML = italianFood[key].description;

   c('.pizzaInfo--actualPrice').innerHTML = `R$ ${italianFood[key].price.toFixed(2)}`;
   ...
   cs('.pizzaInfo--size').forEach((size,sizeIndex)=>{
        ...
        size.querySelector('span').innerHTML = italianFood[key].sizes[sizeIndex]
...
c('.pizzaInfo--addButton').addEventListener('click',()=>{
            ...
            // Quantas pizzas ?
                ...
                let identifier = italianFood[modalKey].id+'@'+size;
...
c('.menu-closer').addEventListener('click',()=>{
    ...
    function updateCart(){
        ...
        for(let i in cart) {
            let pizzaItem = italianFood.find((item)=>{
